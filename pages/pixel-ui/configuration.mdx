...
## Progress Wheel
The progress wheel which PIXEL UI presents during asset downloads can be changed to the server owner's preference with the `ProgressImageURL` setting.
By default the value is set to [`https://pixel-cdn.lythium.dev/i/47qh6kjjh`](https://pixel-cdn.lythium.dev/i/47qh6kjjh), which is the image seen below:

<Image
	src={progressWheelImage} alt="Progress Wheel"
	className="p-2 mt-4 bg-neutral-900 dark:bg-transparent rounded-lg border dark:border-zinc-800"
/>
